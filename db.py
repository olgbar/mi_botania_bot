...
class PlantRepository:
        ...
        with self._get_conn() as conn:
            with conn.cursor() as cur:
                cur.execute("""
                    ...
                    DO UPDATE SET care = EXCLUDED.care,
    ...
    def get_plants(self, user_id):
        with self._get_conn() as conn:
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
                cur.execute("""
                    SELECT name, care, water_every_days
                    FROM plants WHERE user_id = %s
                """, (user_id,))
                return cur.fetchall()
    ...
    def get_reminders(self, user_id):
        with self._get_conn() as conn:
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
                cur.execute("""
                    SELECT plant_name, days_interval
                    FROM reminders
                    WHERE user_id = %s
                """, (user_id,))
                return cur.fetchall()
    ...
    def get_all_reminders(self):
        with self._get_conn() as conn:
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
                cur.execute("""
                    SELECT user_id, plant_name, days_interval
                    FROM reminders
                """)
                return cur.fetchall()
        ...
        with self._get_conn() as conn:
            with conn.cursor() as cur:
                cur.execute("""
                    ...
                    WHERE user_id = %s AND LOWER(plant_name) = LOWER(%s)
                """, (user_id, plant_name))
        ...
        with self._get_conn() as conn:
            with conn.cursor() as cur:
                cur.execute("""
                    ...
                    VALUES (%s, %s)
                    ON CONFLICT (user_id)
                    DO UPDATE SET last_plant = EXCLUDED.last_plant;
                """, (user_id, plant_name))
    ...
    def get_last_plant(self, user_id):
        with self._get_conn() as conn:
            with conn.cursor() as cur:
                cur.execute("""
                    SELECT last_plant FROM user_context WHERE user_id = %s
                """, (user_id,))
                res = cur.fetchone()
